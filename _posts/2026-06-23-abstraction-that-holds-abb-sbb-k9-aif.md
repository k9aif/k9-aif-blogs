...
From that point on, every project I touched started to incorporate that approach. I coupled it with STL in C++, began using Rogue Wave's Tools.h++ — libraries that themselves were built on the same principles of abstraction and reuse. Those are the strong building blocks for Software Engineering. And from then onwards, I have applied OOA, OOD, and Design Patterns everywhere, in every solution I have designed and built. Over thirty-plus years, that idea — that abstraction is the mechanism by which software survives change — has guided how I think about every system.
