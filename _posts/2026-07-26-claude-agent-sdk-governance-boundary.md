...
The instinct was to build the same thing for the Claude Agent SDK: an adapter, a payload mapper, a litellm-bridge equivalent. I started exactly there — read the CrewAI package end to end first, matched its module breakdown, checked what ABBs it actually extends (`BaseOrchestrator`, `BaseAdapter`), and only then started writing.

The parallel held for about half the adapter.
...
So the honest statement is: the Claude Agent SDK's own reasoning loop cannot be routed through `llm_invoke`. Not yet, not with more adapter code — structurally, by what the product is. It's Anthropic's own harness, authenticating and reasoning end to end inside a subprocess K9-AIF doesn't own.
