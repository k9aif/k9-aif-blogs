...
categories: [Architecture, AI, Routing]
---
...
NotDiamond is a **predictive routing service** — a trained meta-model that dynamically selects the best LLM for each query based on quality, cost, or latency signals. Its routing intelligence is learned from data, not configured as rules. It operates as an external API call.
...
NotDiamond, by contrast, returns a session ID per routing call. That ID can be used to submit feedback and track decisions, but there is no server-side session persistence, no model affinity tracking across sessions, and no configurable backend. State management is left entirely to the caller.
...
| Enterprise deployment | Self-hosted, air-gap compatible, PostgreSQL-backed | SaaS only — requires outbound internet access |
...
The K9-AIF Model Router is excellent at **where routing fits** in a governed enterprise architecture — persisted, policy-aware, session-aware, and architecturally isolated behind a stable ABB contract.
