...
- **What tool it can actually use, in the moment.** Before every single tool call runs, the SDK stops and asks the adapter for permission. The adapter checks that request against K9-AIF's own security layer, [k9x_Shield](https://blog.k9x.ai/k9x-satan-proving-shield-holds/), before answering yes or no. Same Router, Orchestrator, and Shield chain that governs everything else in the framework; this adapter just gives it a new place to run.
...
## The other half: how it thinks stays with Claude

Here's where the two designs actually diverge, and no amount of adapter cleverness changes that.
...
- I checked the one setting that looked like it might be a possibility, a low-level connection option. It turned out to control *how* the SDK talks to the Claude process, not *what* answers the questions.
...
The tempting move here would have been to build something that *looked* like it solved this anyway, logging the model's output somewhere and calling it "governed." That would have been the wrong move. A fake safeguard is worse than an honest boundary, because it teaches the next person to trust something that isn't actually there.
...
## What K9-AIF adds around the SDK

The Claude Agent SDK remains responsible for its own agent loop and model inference. The adapter does not try to change that. What it does is bring the SDK's observable actions into the same framework controls used elsewhere in K9-AIF.

Tools exposed to the SDK come from the adapter's registered capability set. Subagents are constructed from that same set and cannot acquire capabilities that were never made available by the adapter. Tool execution passes through the framework governance path, including K9X Shield when configured, while Zero Trust controls can be applied at the surrounding execution boundary.

The integration also participates in K9-AIF's existing monitoring and event mechanisms, rather than creating a separate operational model specifically for Claude Agent SDK.

That is really the purpose of the adapter: preserve the strengths of the Claude Agent SDK while applying as much of the existing K9-AIF architecture around it as the SDK boundary allows.

The model's internal reasoning remains Claude's. The actions that cross the framework boundary remain subject to K9-AIF.

---

## Where this lives

The implementation is in [`k9_adapters/claude_agent_sdk/`](https://github.com/k9aif/k9-aif-framework/tree/main/k9_aif_abb/k9_adapters/claude_agent_sdk), centered on `ClaudeAgentSDKOrchestratorAdapter`, `ClaudeAgentSDKPayloadMapper`, and `K9ClaudeAgentSDKAdapter`. The integration currently targets `claude-agent-sdk==0.2.128`.

The adapter directory also includes a `CLAUDE.md` documenting the implementation decisions and SDK behavior verified during development, including the `_internal/query.py` path used to confirm that subagent tool requests pass through the same permission callback.

The goal was never to make the model reason the way K9-AIF wants. It's to make sure that whatever it decides to do next has to clear a gate first — and to be explicit about where that gate does not reach.
...
- K9X SATAN, Proving Shield Actually Holds: [blog.k9x.ai/k9x-satan-proving-shield-holds](https://blog.k9x.ai/k9x-satan-proving-shield-holds/)
